...
4. Provide clear analysis reasoning""",
        ...
        print(f"  🌐 API configuration: Alibaba Cloud DashScope - GLM model")
        print(f"  Model version: {self.model_name}")
        print(f"  ⚠️  Note: GLM model requires streaming mode")
    ...
    def analyze_with_independent_voting(self, text: str, labels: List[str], human_label: str = None) -> ModelPrediction:
        ...
        if label1 == label2:
            ...
            if len(majority_labels) == 1 and max_count >= 2:
                ...
                agreement_status += f" → Agreement reached after 3rd attempt"
    ...
    def _independent_vote(self, text: str, labels: List[str], attempt_num: int,
        ...
        """Independent vote, prioritizing new categories - using streaming mode"""
        ...
        try:
            ...
            # GLM model requires streaming mode
            completion = self.client.chat.completions.create(
                ...
                max_tokens=300,
                stream=True,  # GLM model requires streaming mode
                extra_body={"stream_mode": "normal"}  # Add extra parameter
            )
    ...
    def _build_vote_prompt_with_priority(self, text: str, labels: List[str], attempt_num: int,
        ...
        if attempt_num == 1:
            ...
            return f"""As a User Experience Designer, please independently analyze the following requirement:
...
Please analyze independently from user experience perspectives, and provide your judgment:
...
[Confidence] [Decimal between 0.0-1.0]"""
            ...
            return f"""As a User Experience Designer, please comprehensively consider previous analyses and provide a final judgment:
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str, ece: float, pearson_corr: float):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("GLM Model - Independent Voting Classification System Analysis Report\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """Print statistics"""
        print(f"\n{'=' * 60}")
        print("📊 GLM Model - Independent Voting Classification Statistics")
...
# Keep the original DataLoader class (unchanged)
class DataLoader:
    """Data loader class"""
...
def main():
    ...
    print("=" * 80)
    print("🎨 GLM Model - Independent Voting Classification System (Prioritize New Categories)")
    ...
    print("🌐 Model Used: Zhipu AI GLM-4.5 (via Alibaba Cloud DashScope)")
    print("⚠️  Note: Using streaming mode for GLM model compatibility")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n🚀 Creating single-role GLM model client...")
        single_role_client = SingleRoleGLMClient(
            name="GLM-UXDesigner-PriorityNew",
            model_name="glm-4.5"
        )
    except Exception as e:
        print(f"❌ Failed to create GLM model client: {e}")
        return
    ...
    print(f"\n{'=' * 80}")
    print("🎉 GLM Model independent voting classification processing completed!")
