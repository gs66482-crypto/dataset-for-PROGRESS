...
class ModelPrediction:
    ...
    match_result: bool = False  # Whether matches human annotation
    total_attempts: int = 0  # Total number of attempts
    final_reason: str = ""  # Reason for final inconsistency
...
class SingleRoleDeepSeekClient:
    """Single-role DeepSeek model client class - Only User Experience Designer"""
    ...
    def __init__(self, name: str = "deepseek-chat", model_name: str = "deepseek-chat"):
        ...
        # Use DeepSeek official API endpoint
        self.client = OpenAI(
            api_key=self.api_key,
            base_url="https://api.deepseek.com/v1"
        )

        # Only define User Experience Designer role
        self.role = {
...
1. Analyze requirements deeply from user experience perspectives
...
4. Provide clear analysis reasoning""",
        ...
        print(f"✅ Successfully created single-role DeepSeek model client: {self.name}")
        print(f"  Professional role used: {self.role['name']}")
        print(f"  🔄 Attempt mechanism: First vote, compare with label, retry if inconsistent, max 3 attempts")
        print(f"  📝 Inconsistency reason recording: If still inconsistent after 3 attempts, record reason")
        print(f"  API configuration: DeepSeek Official API")
    ...
    def single_vote(self, text: str, labels: List[str], attempt_num: int = 1) -> Dict:
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=200,
                stream=False  # DeepSeek supports non-streaming mode
            )
            ...
            if predicted_label:
                print(f"      ✅ Vote {attempt_num} result: {predicted_label} (confidence: {confidence:.3f})")
                return {
                ...
                print(f"      ⚠️ Vote {attempt_num}: {fallback_label} (extraction failed, using default)")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
    ...
    def analyze_with_retry(self, text: str, labels: List[str], human_label: str = None,
        ...
        """Analysis with retry mechanism: First vote, compare with label, retry if inconsistent, max 3 attempts"""
        ...
        # Check if matches human annotation
        if human_label:
            match_result = (attempt1["predicted_label"] == human_label)

            if match_result:
                print(f"    ✅ First vote already matches human annotation!")
                ...
                if match_result:
                    ...
                    if match_result:
                        ...
                        print(f"    ❌ Still inconsistent after 3 votes, extracting reason...")
                        final_label = attempt3["predicted_label"]
                        final_confidence = attempt3["confidence"]

                        # Extract inconsistency reason
    ...
    def _rethinking_vote(self, text: str, labels: List[str], human_label: str,
        ...
        print(f"    🔄 Vote {attempt_num} (rethinking)...")
        ...
        try:
            ...
            if predicted_label:
                print(f"      ✅ Rethinking vote {attempt_num} result: {predicted_label} (confidence: {confidence:.3f})")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
                    "predicted_label": predicted_label,
                ...
                fallback_label = labels[0] if labels else "Unknown"
                print(f"      ⚠️ Rethinking vote {attempt_num}: {fallback_label} (extraction failed)")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
                    "predicted_label": fallback_label,
            ...
            print(f"      ❌ Rethinking vote {attempt_num} exception: {str(e)[:100]}")
            fallback_label = labels[0] if labels else "Unknown"
            return {
                "attempt_number": attempt_num,
                "response": str(e),
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        """Extract disagreement reason"""
        try:
            # Use last attempt's response to extract reason
            ...
            return f"UX Designer analyzed from user experience perspective as '{model_label}', while human annotation is '{human_label}'. After {len(attempts)} attempts, perspective difference remains."
...
Please think from {role_name}'s perspective and select the most appropriate classification label."""
        ...
        response_format = f"""Please reply in the following format:
...
Please re-analyze from {role_name}'s professional perspective: why might the human expert have a different annotation?
Then provide what you believe is the most appropriate classification label.
...
Your previous classification was: [{previous_label}]
However, the human expert's annotation has consistently been: [{human_label}]

From {role_name}'s professional perspective:
1. Provide what you believe is the most appropriate classification label
...
[Reason Analysis] [Explain why it differs from human annotation, within 50 words]"""
    ...
    def _extract_confidence_from_response(self, response: str) -> float:
        ...
        # If not found, try to find numbers between 0-1
...
class SingleRoleProcessor:
    """Single-role model processor - Only User Experience Designer"""
    ...
    def process_dataset(self, dataset_with_labels: List[Dict], labels: List[str],
        ...
        print(f"🔄 Attempt mechanism: 1 vote → Compare → Retry if inconsistent (max 3 attempts)")
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("DeepSeek Model - User Experience Designer Single-Role Classification System Analysis Report\n")
            f.write("Special Configuration: First vote, compare with label, retry if inconsistent, max 3 attempts\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("  - Final confidence uses the last vote's confidence\n\n")
            ...
            for attempts, count in attempt_distribution.items():
                if count > 0:
                    ...
                    f.write(f"  {attempts} attempts: {count} items ({percentage:.1f}%)\n")
            f.write("\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """Print statistics"""
        print(f"\n{'=' * 60}")
        print("📊 DeepSeek Model - User Experience Designer Single-Role Classification Statistics")
        print(f"Special Configuration: First vote, compare with label, retry if inconsistent, max 3 attempts")
        print(f"{'=' * 60}")
        ...
        for attempts, count in attempt_distribution.items():
            if count > 0:
                percentage = count / len(results) * 100
                print(f"  {attempts} attempts: {count} items ({percentage:.1f}%)")
...
class DataLoader:
    ...
    def load_dataset_with_labels(file_path: str) -> List[Dict]:
        ...
        try:
            ...
            if label_col:
                print(f"  {sum(1 for item in dataset_with_labels if 'label' in item)} items have annotations")
...
def main():
    ...
    print("=" * 80)
    print("🎨 DeepSeek Model - User Experience Designer Single-Role Classification System")
    print("Special Configuration: First vote, compare with label column in dataset.xlsx")
    print("🔄 Retry Mechanism: If inconsistent, retry second vote, max 3 attempts")
    ...
    if api_key:
        print(f"✅ DEEPSEEK_API_KEY is set (length: {len(api_key)})")
    else:
        print(f"❌ DEEPSEEK_API_KEY is not set")
        print(f"💡 Please set the DEEPSEEK_API_KEY environment variable")
        return
    ...
    try:
        # Create single-role DeepSeek model client
    ...
    if labeled_count == 0:
        print(f"\n⚠️  Warning: Dataset does not have a label column, annotation comparison will not be possible")
        print(f"  Retry mechanism will not be triggered")

    # Execute single-role model processing
    print(f"\n🚀 Starting DeepSeek Model UX Designer single-role classification processing...")
    print(f"🎯 Note: After each vote, it will be compared with the label column in the dataset")
    print(f"🔄 Retry mechanism: Inconsistent → Retry → Max 3 attempts → Record reason")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 DeepSeek Model UX Designer single-role classification processing completed!")
    print(f"📊 Results file: {output_file}")
    print(f"📄 Detailed report: {output_file.replace('.xlsx', '_detailed_report.txt')}")
    print(f"🔄 Retry mechanism: 1 vote → Compare with label → Retry if inconsistent (max 3 attempts)")
    print(f"📝 Reason recording: If still inconsistent after 3 attempts → Record detailed reason")
    print("=" * 80)
