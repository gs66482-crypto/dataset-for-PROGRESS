...
class ModelPrediction:
    """存储模型预测结果"""
    label: str
    confidence: float  # 最终置信度（最后一次投票的置信度）
    model_name: str
...
4. 提供清晰的分析理由""",
        ...
        try:
            ...
            if predicted_label:
                ...
                return {
                    ...
                    "confidence": confidence,  # LLM自我评估的置信度
    ...
    def analyze_with_retry(self, text: str, labels: List[str], human_label: str = None,
        ...
        if human_label:
            ...
            if match_result:
                ...
                final_confidence = attempt1["confidence"]  # 使用第一次投票的置信度
            else:
                ...
                if match_result:
                    ...
                    final_confidence = attempt2["confidence"]  # 使用第二次投票的置信度
                else:
                    ...
                    if match_result:
                        ...
                        final_confidence = attempt3["confidence"]  # 使用第三次投票的置信度
                    else:
        ...
        # 打印置信度说明
        print(f"    📊 置信度计算:")
        print(f"      最终使用第{len(attempts)}次投票的置信度: {final_confidence:.3f}")
        for i, attempt in enumerate(attempts):
            print(f"      第{i + 1}次投票置信度: {attempt['confidence']:.3f}")
        ...
        print(f"  最终置信度: {final_confidence:.3f} (第{len(attempts)}次投票)")
        ...
        return ModelPrediction(
            label=final_label,
            confidence=final_confidence,  # 使用最后一次投票的置信度
    ...
    def _rethinking_vote(self, text: str, labels: List[str], human_label: str,
        ...
        try:
            ...
            predicted_label = self._extract_label_from_response(response_content, labels)
            confidence = self._extract_confidence_from_response(response_content)  # LLM自我评估的置信度
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        ...
        try:
            ...
            # 如果没有明确的原因，生成一个概括性原因
            return f"产品负责人从商业价值角度分析为'{model_label}'，而人类标注为'{human_label}'。经过{len(attempts)}次尝试，仍然存在视角差异。"
...
class SingleRoleProcessor:
    ...
    def process_dataset(self, dataset_with_labels: List[Dict], labels: List[str],
        ...
        print(f"🎭 角色配置: Product Owner")
        print(f"🔄 尝试机制: 1次 → 核对 → 不一致再投(最多3次)")
        print(f"📊 置信度计算: 使用最后一次投票的置信度")
    ...
    def save_results(self, results: List[DataPoint], output_file: str):
        ...
        for i, data_point in enumerate(results):
            ...
            if data_point.prediction:
                confidence = data_point.prediction.confidence  # 最终置信度（最后一次投票的置信度）
            ...
            row_data = {
                ...
                '置信度来源': f"第{data_point.prediction.total_attempts}次投票" if data_point.prediction else "",
                '备注': f"共{data_point.prediction.total_attempts}次尝试，使用最后一次置信度" if data_point.prediction else ""
            }
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("豆包模型-Product Owner单角色分类系统分析报告\n")
            f.write("特殊配置: 先投一次，与label核对，不一致再投，最多3次\n")
            f.write("置信度: 使用最后一次投票的置信度\n")
            f.write("=" * 80 + "\n\n")
            ...
            if inconsistencies:
                ...
                for inc in inconsistencies:
                    ...
                    f.write(f"  模型预测: {inc['model']} (最终置信度: {inc['confidence']:.3f})\n")
...
def main():
    ...
    if api_key:
        print(f"✅ DOUBAO_API_KEY 已设置 (长度: {len(api_key)})")
    else:
        print(f"❌ DOUBAO_API_KEY 未设置")
        print(f"💡 请设置环境变量 DOUBAO_API_KEY")
        return
    ...
    for i, data_point in enumerate(results[:3]):
        ...
        if data_point.human_label:
            ...
            print(
                f"   最终置信度: {data_point.prediction.confidence:.3f} (第{data_point.prediction.total_attempts}次投票)")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 豆包模型Product Owner单角色分类处理完成!")
