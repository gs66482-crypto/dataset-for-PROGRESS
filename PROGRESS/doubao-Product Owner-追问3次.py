...
class ModelPrediction:
    ...
    label: str
    confidence: float  # Final confidence (confidence of the last vote)
    model_name: str
...
class SingleRoleDoubaoClient:
    """Single-role Doubao model client class - Product Owner only"""
    ...
    def __init__(self, name: str = "doubao-seed-1-6-250615", model_name: str = "doubao-seed-1-6-250615"):
        ...
        # Define Product Owner role only
        self.role = {
            "name": "Product Owner",
...
Please follow these requirements for analysis:
...
4. Provide clear analysis reasoning""",
        ...
        print(f"  📊 Confidence: Uses confidence from the last vote")
        print(f"  API Configuration: Volcano Engine Doubao")
    ...
    def single_vote(self, text: str, labels: List[str], attempt_num: int = 1) -> Dict:
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=200,
                stream=False  # Doubao supports non-streaming mode
            )
            ...
            # Extract confidence - LLM self-assessed confidence
            ...
            if predicted_label:
                ...
                return {
                    ...
                    "confidence": confidence,  # LLM self-assessed confidence
    ...
    def analyze_with_retry(self, text: str, labels: List[str], human_label: str = None,
        ...
        if human_label:
            ...
            if match_result:
                ...
                final_confidence = attempt1["confidence"]  # Use confidence from first vote
            else:
                ...
                if match_result:
                    ...
                    final_confidence = attempt2["confidence"]  # Use confidence from second vote
                else:
                    ...
                    if match_result:
                        ...
                        final_confidence = attempt3["confidence"]  # Use confidence from third vote
                    else:
        ...
        print(f"      Final confidence uses vote {len(attempts)}: {final_confidence:.3f}")
        for i, attempt in enumerate(attempts):
            print(f"      Vote {i + 1} confidence: {attempt['confidence']:.3f}")
        ...
        print(f"  Final confidence: {final_confidence:.3f} (vote {len(attempts)})")
        ...
        return ModelPrediction(
            label=final_label,
            confidence=final_confidence,  # Use confidence from last vote
    ...
    def _rethinking_vote(self, text: str, labels: List[str], human_label: str,
        ...
        try:
            ...
            predicted_label = self._extract_label_from_response(response_content, labels)
            confidence = self._extract_confidence_from_response(response_content)  # LLM self-assessed confidence
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        ...
        try:
            ...
            return f"The Product Owner analyzed from a business value perspective as '{model_label}', while the human annotation is '{human_label}'. After {len(attempts)} attempts, there remains a perspective difference."
...
class SingleRoleProcessor:
    """Single-role model processor - Product Owner only"""
    ...
    def process_dataset(self, dataset_with_labels: List[Dict], labels: List[str],
        ...
        print(f"📊 Confidence calculation: Uses confidence from the last vote")
    ...
    def save_results(self, results: List[DataPoint], output_file: str):
        ...
        for i, data_point in enumerate(results):
            ...
            if data_point.prediction:
                confidence = data_point.prediction.confidence  # Final confidence (confidence from last vote)
            ...
            row_data = {
                ...
                'Confidence Source': f"Vote {data_point.prediction.total_attempts}" if data_point.prediction else "",
                'Notes': f"Total {data_point.prediction.total_attempts} attempts, using last vote confidence" if data_point.prediction else ""
            }
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Doubao Model - Product Owner Single-Role Classification System Analysis Report\n")
            f.write("Special Configuration: Vote once, compare with label, retry if inconsistent, up to 3 times\n")
            f.write("Confidence: Uses confidence from the last vote\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("  - No weighted averaging, directly uses the last vote's confidence value\n\n")
            ...
            if inconsistencies:
                ...
                for inc in inconsistencies:
                    ...
                    f.write(f"  Model Prediction: {inc['model']} (Final Confidence: {inc['confidence']:.3f})\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"\n{'=' * 60}")
        print("📊 Doubao Model - Product Owner Single-Role Classification Statistics")
...
def main():
    ...
    output_file = "doubao_PO_retry_mechanism.xlsx"

    data_loader = DataLoader()

    print("=" * 80)
    print("🎯 Doubao Model - Product Owner Single-Role Classification System")
    ...
    print("📊 Confidence: Uses confidence from the last vote")
    print("=" * 80)
    ...
    if api_key:
        print(f"✅ DOUBAO_API_KEY is set (length: {len(api_key)})")
    else:
        print(f"❌ DOUBAO_API_KEY is not set")
        print(f"💡 Please set the DOUBAO_API_KEY environment variable")
        return
    ...
    try:
        # Create single-role Doubao model client
    ...
    print(f"\n🚀 Starting Doubao model Product Owner single-role classification processing...")
    ...
    for i, data_point in enumerate(results[:3]):
        ...
        if data_point.human_label:
            ...
            print(f"   Final Confidence: {data_point.prediction.confidence:.3f} (vote {data_point.prediction.total_attempts})")

            if data_point.prediction.attempts:
                print(f"   Voting History:")
                for attempt in data_point.prediction.attempts:
                    print(f"     Attempt {attempt['attempt_number']}: {attempt['predicted_label']} (confidence: {attempt['confidence']:.3f})")

            if data_point.prediction.final_reason and not data_point.prediction.match_result:
                print(f"   Reason for Inconsistency: {data_point.prediction.final_reason}")
        else:
            print(f"   Model Prediction: {data_point.final_label}")
            print(f"   Final Confidence: {data_point.prediction.confidence:.3f} (vote {data_point.prediction.total_attempts})")
            print(f"   Attempts: {data_point.prediction.total_attempts}")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 Doubao Model Product Owner Single-Role Classification Processing Complete!")
