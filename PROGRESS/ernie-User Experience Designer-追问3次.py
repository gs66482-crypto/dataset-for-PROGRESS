...
class SingleRoleERNIEClient:
    """单角色ERNIE-4.5模型客户端类 - 仅User Experience Designer"""
    ...
    def __init__(self, name: str = "ERNIE-4.5", model_name: str = "baidu/ERNIE-4.5-300B-A47B"):
        ...
        # 初始化ERNIE-4.5客户端（通过SiliconFlow）
        api_key = os.getenv("SILICON_FLOW_API_KEY")
        base_url = os.getenv("SILICON_FLOW_BASE_URL")

        if not api_key:
            raise ValueError("SILICON_FLOW_API_KEY环境变量未设置")
        if not base_url:
            raise ValueError("SILICON_FLOW_BASE_URL环境变量未设置")
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("ERNIE-4.5模型-User Experience Designer单角色分类系统分析报告\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """打印统计信息"""
        print(f"\n{'=' * 60}")
        print("📊 ERNIE-4.5模型-User Experience Designer单角色分类统计信息")
...
def main():
    ...
    print("=" * 80)
    print("🎨 ERNIE-4.5模型-User Experience Designer单角色分类系统")
    ...
    if api_key:
        print(f"✅ SILICON_FLOW_API_KEY 已设置 (长度: {len(api_key)})")
    else:
        print(f"❌ SILICON_FLOW_API_KEY 未设置")
        print(f"💡 请设置环境变量 SILICON_FLOW_API_KEY")
        return

    if base_url:
        print(f"✅ SILICON_FLOW_BASE_URL 已设置")
    else:
        print(f"❌ SILICON_FLOW_BASE_URL 未设置")
        print(f"💡 请设置环境变量 SILICON_FLOW_BASE_URL")
        print(f"💡 通常为: https://api.siliconflow.cn/v1")
        return
    ...
    # 执行单角色模型处理
    print(f"\n🚀 开始ERNIE-4.5模型UX Designer单角色分类处理...")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 ERNIE-4.5模型UX Designer单角色分类处理完成!")
