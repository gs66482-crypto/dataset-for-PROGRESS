...
class ModelPrediction:
    ...
    confidence: float
    model_name: str
    attempts: List[Dict] = None  # Records of each attempt
    human_label: str = None  # Human annotation
    match_result: bool = False  # Whether it matches the human annotation
...
class DataPoint:
    ...
    human_label: str = None  # Human annotated label
...
class ResearchPaperConfidenceCalculator:
    ...
    def calculate_confidence(self, vote_counts: Dict[str, int]) -> Dict[str, Any]:
        ...
        N = 5  # Total number of votes fixed at 5
...
class SingleRoleHunyuanClient:
    """Single-role Tencent Hunyuan model client class - Software Tester only"""

    def __init__(self, name: str = "Tencent Hunyuan", model_name: str = "hunyuan-turbos-latest"):
        self.name = name
        ...
        if not self.api_key:
            raise ValueError("HUNYUAN_API_KEY environment variable not set")

        # Define only the Software Tester role
        self.role = {
            "name": "Software Tester",
            "system_prompt": """You are a Software Tester, focusing on quality assurance, test design, and defect prevention. You analyze requirements from the perspectives of testability, test case design, boundary conditions, error handling, and quality standards. You focus on how requirements can be verified, test coverage, potential risk points, and quality assurance measures. Your analysis emphasizes how to "verify" whether requirements are correctly implemented.

Please analyze according to the following requirements:
1. Conduct in-depth analysis of requirements from a software testing and quality assurance perspective
...
4. Provide clear analysis reasoning""",
        ...
        print(f"✅ Successfully created single-role Tencent Hunyuan model client: {self.name}")
        print(f"  Professional role used: {self.role['name']}")
        print(f"  🔄 Attempt mechanism: First vote, check against label, retry if inconsistent, up to 3 times")
        print(f"  📝 Inconsistency reason recording: Record reason if still inconsistent after 3 attempts")
        print(f"  API configuration: Tencent Hunyuan")
    ...
    def single_vote(self, text: str, labels: List[str], attempt_num: int = 1) -> Dict:
        ...
        try:
            ...
            if predicted_label:
                print(f"      ✅ {attempt_num}th vote result: {predicted_label} (confidence: {confidence:.3f})")
                return {
                ...
                print(f"      ⚠️ {attempt_num}th vote: {fallback_label} (extraction failed, using default)")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
            ...
            print(f"      ❌ {attempt_num}th vote exception: {str(e)[:100]}")
            fallback_label = labels[0] if labels else "Unknown"
            return {
                "attempt_number": attempt_num,
                "response": str(e),
    ...
    def analyze_with_retry(self, text: str, labels: List[str], human_label: str = None,
        ...
        """Analysis with retry mechanism: First vote, check against label, retry if inconsistent, up to 3 times"""
        ...
        # Check if it matches the human annotation
        if human_label:
            match_result = (attempt1["predicted_label"] == human_label)

            if match_result:
                print(f"    ✅ Matched human annotation on first vote!")
                ...
                if match_result:
                    ...
                    if match_result:
                        ...
                        print(f"    📝 Inconsistency reason: {final_reason[:100]}...")
        else:
            # If there is no human annotation, use the first vote result directly
            print(f"    ℹ️ No human annotation for comparison, using first vote result")
            final_label = attempt1["predicted_label"]
            final_confidence = attempt1["confidence"]
            match_result = True  # Default to consistent since there's no annotation
        ...
        if final_reason and not match_result:
            print(f"  Inconsistency reason: {final_reason}")
    ...
    def _rethinking_vote(self, text: str, labels: List[str], human_label: str,
        ...
        print(f"    🔄 {attempt_num}th vote (rethinking)...")
        ...
        try:
            ...
            if predicted_label:
                print(f"      ✅ {attempt_num}th rethinking result: {predicted_label} (confidence: {confidence:.3f})")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
                    "predicted_label": predicted_label,
                ...
                fallback_label = labels[0] if labels else "Unknown"
                print(f"      ⚠️ {attempt_num}th rethinking: {fallback_label} (extraction failed)")
                return {
                    "attempt_number": attempt_num,
                    "response": response_content,
                    "predicted_label": fallback_label,
            ...
            print(f"      ❌ {attempt_num}th rethinking exception: {str(e)[:100]}")
            fallback_label = labels[0] if labels else "Unknown"
            return {
                "attempt_number": attempt_num,
                "response": str(e),
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        ...
        try:
            ...
            # If no explicit reason, generate a summary reason
            return f"Software Tester analyzed from quality assurance perspective as '{model_label}', while human annotation is '{human_label}'. After {len(attempts)} attempts, perspective differences remain."
    ...
    def _build_single_vote_prompt(self, text: str, labels: List[str],
        ...
        labels_section = f"""Optional classification labels (strictly select one from the following labels):

{chr(10).join([f'• {label}' for label in labels])}

Important: Please ensure you return exactly the label name as shown above, do not modify, abbreviate, or add any extra content."""
...
Please think from the perspective of {role_name} and select the most appropriate classification label."""
...
【Confidence】[Your confidence level in this classification, a decimal between 0.0-1.0, e.g., 0.85]"""
...
As {role_name}, you need to rethink your previous classification.

Requirement description:
"{text}"

Your previous classification was: 【{previous_label}】
But the human expert's annotation is: 【{human_label}】

Please reanalyze from {role_name}'s professional perspective, why might the human expert have a different annotation?
Then provide what you consider to be the most appropriate classification label.

Please respond in the following format:
【{role_name} Rethinking】[Your analysis]
【Classification Label】[Must select one from the above labels]
【Confidence】[Your confidence level in this classification, a decimal between 0.0-1.0]"""
    ...
    def _build_final_rethinking_prompt(self, text: str, labels: List[str],
                                       human_label: str, previous_label: str,
                                       role_info: Dict) -> str:
        """Build final rethinking prompt (request reason analysis)"""
...
{chr(10).join([f'• {label}' for label in labels])}"""
        ...
        final_guidance = f"""
As {role_name}, this is your final opportunity to rethink.

Requirement description:
"{text}"

Your previous classification was: 【{previous_label}】
But the human expert's annotation has consistently been: 【{human_label}】

From {role_name}'s professional perspective:
1. Provide what you consider to be the most appropriate classification label
...
【Reason Analysis】[Explain why it differs from the human annotation, within 50 words]"""
    ...
    def _extract_confidence_from_response(self, response: str) -> float:
        ...
        # If not explicitly found, try to find numbers between 0-1
...
class SingleRoleProcessor:
    """Single-role model processor - Software Tester only"""

    def __init__(self, single_role_client: SingleRoleHunyuanClient):
    ...
    def process_dataset(self, dataset_with_labels: List[Dict], labels: List[str],
        ...
        print(f"Starting to process {len(dataset_with_labels)} software requirements with labels...")
        print(f"Number of classification labels: {len(labels)}")
        print(f"🎭 Role configuration: Software Tester")
        print(f"🔄 Attempt mechanism: 1 vote → check → if inconsistent, retry (up to 3 times)")
        ...
        for i, item in enumerate(dataset_with_labels):
            ...
            human_label = item.get('label', None)  # Note: using 'label' column here
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Tencent Hunyuan Model - Software Tester Single-Role Classification System Analysis Report\n")
            f.write("Special Configuration: First vote, check against label, retry if inconsistent, up to 3 times\n")
            f.write("=" * 80 + "\n\n")

            f.write(f"Analysis Time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Total Requirements: {len(results)}\n")
            f.write("Professional Role Used:\n")
            f.write("1. Software Tester\n\n")

            f.write("🔄 Attempt Mechanism:\n")
            f.write("  1. First vote\n")
            f.write("  2. Check against the label column in the dataset\n")
            f.write("  3. If inconsistent, conduct second vote\n")
            f.write("  4. Check again, if inconsistent and under 3 attempts, conduct third vote\n")
            f.write("  5. If still inconsistent after 3 attempts, record the reason\n\n")

            f.write("🎯 Confidence Mechanism:\n")
            f.write("  - Each vote includes LLM self-assessed confidence\n")
            f.write("  - Final confidence uses the confidence of the last vote\n\n")
            ...
            if total_with_human_label > 0:
                ...
                f.write("No human annotated data available for consistency rate statistics\n\n")
            ...
            f.write("🔄 Attempt Count Distribution:\n")
            for attempts, count in attempt_distribution.items():
                if count > 0:
                    percentage = count / len(results) * 100
                    f.write(f"  {attempts} attempt(s): {count} items ({percentage:.1f}%)\n")
            f.write("\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"\n{'=' * 60}")
        print("📊 Tencent Hunyuan Model - Software Tester Single-Role Classification Statistics")
        print(f"Special Configuration: First vote, check label, retry if inconsistent, up to 3 times")
        print(f"{'=' * 60}")

        print(f"Total Data Processed: {len(results)} requirements")
        print(f"Role Used: Software Tester")
        ...
        for attempts, count in attempt_distribution.items():
            if count > 0:
                percentage = count / len(results) * 100
                print(f"  {attempts} attempt(s): {count} items ({percentage:.1f}%)")
    ...
    def load_dataset_with_labels(file_path: str) -> List[Dict]:
        ...
        try:
            ...
            if 'requirement' not in df.columns:
                print(f"⚠️  'requirement' column not found in file, trying to use first column as requirement content")
                requirement_col = df.columns[0]
            else:
                requirement_col = 'requirement'

            if 'label' not in df.columns:
                print(f"⚠️  'label' column not found in file, label comparison will not be possible")
                label_col = None
            else:
                label_col = 'label'
            ...
            print(f"✅ Successfully loaded {len(dataset_with_labels)} test requirements with labels from {os.path.basename(file_path)}")
            if label_col:
...
def main():
    ...
    dataset_file = "dataset.xlsx"  # Must contain 'requirement' and 'label' columns
    concept_file = "1123Concept.xlsx"
    examples_file = "1122RequirementExamples.xlsx"
    output_file = "hunyuan_SoftwareTester_retry_mechanism.xlsx"

    data_loader = DataLoader()

    print("=" * 80)
    print("🎨 Tencent Hunyuan Model - Software Tester Single-Role Classification System")
    print("Special Configuration: First vote, check against label column in dataset.xlsx")
    print("🔄 Retry Mechanism: If inconsistent, retry for a second vote, up to 3 times")
    ...
    if api_key:
        print(f"✅ HUNYUAN_API_KEY is set (length: {len(api_key)})")
    else:
        print(f"❌ HUNYUAN_API_KEY is not set")
        print(f"💡 Please set the environment variable HUNYUAN_API_KEY")
        return
    ...
    try:
        # Create single-role Tencent Hunyuan model client
        print(f"\n🚀 Creating single-role model client...")
        single_role_client = SingleRoleHunyuanClient(
            name="Tencent Hunyuan-SoftwareTester",
    ...
    if labeled_count == 0:
        print(f"\n⚠️  Warning: No label column in the dataset, label comparison will not be possible")
        print(f"  Retry mechanism will not be triggered")

    # Execute single-role model processing
    print(f"\n🚀 Starting Tencent Hunyuan Model Software Tester Single-Role Classification Processing...")
    print(f"🎯 Note: After each vote, it will check against the label column in the dataset")
    ...
    for i, data_point in enumerate(results[:3]):
        ...
        if data_point.human_label:
            print(f"   True Label: {data_point.human_label}")
            print(f"   Model Prediction: {data_point.final_label}")
            print(f"   Consistent: {'✅ Yes' if data_point.prediction.match_result else '❌ No'}")
            print(f"   Attempts: {data_point.prediction.total_attempts}")

            if data_point.prediction.attempts:
                print(f"   Vote History:")
                for attempt in data_point.prediction.attempts:
                    print(
                        f"     Attempt {attempt['attempt_number']}: {attempt['predicted_label']} (Confidence: {attempt['confidence']:.3f})")

            if data_point.prediction.final_reason and not data_point.prediction.match_result:
                print(f"   Inconsistency Reason: {data_point.prediction.final_reason}")
        else:
            print(f"   Model Prediction: {data_point.final_label}")
            print(f"   Confidence: {data_point.prediction.confidence:.3f}")
            print(f"   Attempts: {data_point.prediction.total_attempts}")

    print(f"\n{'=' * 80}")
    print("🎉 Tencent Hunyuan Model Software Tester Single-Role Classification Processing Complete!")
    print(f"📊 Result file: {output_file}")
    print(f"📄 Detailed report: {output_file.replace('.xlsx', '_Detailed_Report.txt')}")
    print(f"🔄 Retry Mechanism: 1 vote → Check label → Retry if inconsistent (up to 3 times)")
