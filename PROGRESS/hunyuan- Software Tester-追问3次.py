...
class SingleRoleHunyuanClient:
    """单角色腾讯混元模型客户端类 - 仅Software Tester"""

    def __init__(self, name: str = "腾讯混元", model_name: str = "hunyuan-turbos-latest"):
        self.name = name
        ...
        if not self.api_key:
            raise ValueError("HUNYUAN_API_KEY环境变量未设置")

        # 只定义Software Tester角色
        self.role = {
...
4. 提供清晰的分析理由""",
        ...
        print(f"✅ 成功创建单角色腾讯混元模型客户端: {self.name}")
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        ...
        try:
            ...
            # 如果没有明确的原因，生成一个概括性原因
            return f"软件测试工程师从质量保证角度分析为'{model_label}'，而人类标注为'{human_label}'。经过{len(attempts)}次尝试，仍然存在视角差异。"
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("腾讯混元模型-Software Tester单角色分类系统分析报告\n")
            ...
            f.write("使用的专业角色:\n")
            f.write("1. Software Tester (软件测试工程师)\n\n")
...
def main():
    ...
    print("=" * 80)
    print("🎨 腾讯混元模型-Software Tester单角色分类系统")
    ...
    if api_key:
        print(f"✅ HUNYUAN_API_KEY 已设置 (长度: {len(api_key)})")
    else:
        print(f"❌ HUNYUAN_API_KEY 未设置")
        print(f"💡 请设置环境变量 HUNYUAN_API_KEY")
        return
    ...
    # 执行单角色模型处理
    print(f"\n🚀 开始腾讯混元模型Software Tester单角色分类处理...")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 腾讯混元模型Software Tester单角色分类处理完成!")
