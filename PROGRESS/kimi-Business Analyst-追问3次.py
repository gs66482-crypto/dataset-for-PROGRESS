...
class SingleRoleKimiClient:
    """单角色Kimi模型客户端类 - 仅Business Analyst"""
...
4. 提供清晰的分析理由""",
    ...
    def _extract_disagreement_reason(self, text: str, model_label: str, human_label: str, attempts: List[Dict]) -> str:
        ...
        try:
            ...
            # 如果没有明确的原因，生成一个概括性原因
            return f"业务分析师从业务分析角度判断为'{model_label}'，而人类标注为'{human_label}'。经过{len(attempts)}次尝试，仍然存在视角差异。"
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Kimi-K2模型-Business Analyst单角色分类系统分析报告\n")
            ...
            f.write("使用的专业角色:\n")
            f.write("1. Business Analyst (业务分析师)\n\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """打印统计信息"""
        print(f"\n{'=' * 60}")
        print("📊 Kimi-K2模型-Business Analyst单角色分类统计信息")
        print(f"特殊配置: 先投一次，核对label，不一致再投，最多3次")
        print(f"{'=' * 60}")

        print(f"处理总数据量: {len(results)}条需求")
        print(f"使用角色: Business Analyst")
...
def main():
    ...
    print("=" * 80)
    print("📊 Kimi-K2模型-Business Analyst单角色分类系统")
    ...
    if api_key:
        print(f"✅ MOONSHOT_API_KEY 已设置 (长度: {len(api_key)})")
    else:
        print(f"❌ MOONSHOT_API_KEY 未设置")
        print(f"💡 请设置环境变量 MOONSHOT_API_KEY")
        return
    ...
    # 执行单角色模型处理
    print(f"\n🚀 开始Kimi-K2模型Business Analyst单角色分类处理...")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 Kimi-K2模型Business Analyst单角色分类处理完成!")
