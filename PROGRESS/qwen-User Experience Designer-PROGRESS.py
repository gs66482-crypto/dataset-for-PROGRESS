...
class SingleRoleQwenClient:
    """Single-role Qwen model client class - Only User Experience Designer"""
...
4. Provide clear analysis reasoning""",
        ...
        print(f"  API configuration: Alibaba Cloud DashScope - Qwen")
    ...
    def single_vote(self, text: str, labels: List[str], attempt_num: int = 1) -> Dict:
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=200,
                stream=False  # Qwen supports non-streaming mode
            )
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Qwen Model - User Experience Designer Single-Role Classification System Analysis Report\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """Print statistics"""
        print(f"\n{'=' * 60}")
        print("📊 Qwen Model - User Experience Designer Single-Role Classification Statistics")
...
def main():
    ...
    output_file = "qwen_UXDesigner_retry_mechanism.xlsx"

    data_loader = DataLoader()

    print("=" * 80)
    print("🎨 Qwen Model - User Experience Designer Single-Role Classification System")
    ...
    if api_key:
        print(f"✅ DASHSCOPE_API_KEY is set (length: {len(api_key)})")
    else:
        print(f"❌ DASHSCOPE_API_KEY is not set")
        print(f"💡 Please set the DASHSCOPE_API_KEY environment variable")
        return

    if base_url:
        print(f"✅ DASHSCOPE_BASE_URL is set")
    else:
        print(f"❌ DASHSCOPE_BASE_URL is not set")
        print(f"💡 Please set the DASHSCOPE_BASE_URL environment variable")
        return
    ...
    print(f"\n🚀 Starting Qwen Model UX Designer single-role classification processing...")
    ...
    print(f"\n{'=' * 80}")
    print("🎉 Qwen Model UX Designer single-role classification processing completed!")
