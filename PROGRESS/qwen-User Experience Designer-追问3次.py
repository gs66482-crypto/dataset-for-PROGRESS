...
class SingleRoleQwenClient:
    """单角色通义千问模型客户端类 - 仅User Experience Designer"""
    ...
    def __init__(self, name: str = "qwen-plus-2025-07-28", model_name: str = "qwen-plus-2025-07-28"):
        ...
        if not base_url:
            raise ValueError("DASHSCOPE_BASE_URL环境变量未设置")
...
class SingleRoleProcessor:
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("通义千问模型-User Experience Designer单角色分类系统分析报告\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        """打印统计信息"""
        print(f"\n{'=' * 60}")
        print("📊 通义千问模型-User Experience Designer单角色分类统计信息")
...
def main():
    ...
    print("=" * 80)
    print("🎨 通义千问模型-User Experience Designer单角色分类系统")
    ...
    if api_key:
        ...
        print(f"💡 请设置环境变量 DASHSCOPE_API_KEY")
        return

    if base_url:
        print(f"✅ DASHSCOPE_BASE_URL 已设置")
    else:
        print(f"❌ DASHSCOPE_BASE_URL 未设置")
        print(f"💡 请设置环境变量 DASHSCOPE_BASE_URL")
        return
    ...
    print(f"\n{'=' * 80}")
    print("🎉 通义千问模型UX Designer单角色分类处理完成!")
