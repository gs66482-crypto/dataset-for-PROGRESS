...
class SiliconFlowClient:
    """Silicon Flow模型客户端类 - 支持ERNIE-4.5等多种模型"""
    ...
    def __init__(self, name: str = "ERNIE-4.5", model_name: str = "baidu/ERNIE-4.5-300B-A47B"):
        ...
        if not os.getenv('SILICON_FLOW_BASE_URL'):
    ...
    def predict(self, text: str, labels: List[str],
        ...
        for attempt in range(3):  # 重试机制
            try:
                ...
                # 使用OpenAI兼容接口调用Silicon Flow API
...
class SiliconFlowProcessor:
    """Silicon Flow模型处理器 - 语言学者角色"""

    def __init__(self, sf_client: SiliconFlowClient):
        """初始化Silicon Flow模型处理器"""
        self.sf_client = sf_client
        print(f"Silicon Flow语言学者处理器初始化完成，使用模型: {sf_client.name}")
        print(f"模型标识: {sf_client.model_name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用Silicon Flow模型进行预测（单次判断）
            prediction = self.sf_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        """打印统计信息"""
        print(f"\n📊 Silicon Flow语言学者模型统计信息:")
        print(f"处理总数据量: {len(results)}")
        print(f"使用模型: {self.sf_client.name}")
        print(f"模型标识: {self.sf_client.model_name}")
...
def main():
    ...
    output_file = "siliconflow_ernie45语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("Silicon Flow ERNIE-4.5语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建Silicon Flow模型客户端")
        print(f"   模型名称: {sf_client.name}")
        print(f"   模型标识: {sf_client.model_name}")
    except Exception as e:
        print(f"❌ 创建Silicon Flow模型客户端失败: {e}")
        return
    ...
    # 执行Silicon Flow模型处理
    print(f"\n🚀 开始语言学者分析（单次判断模式，随机顺序）...")
    processor = SiliconFlowProcessor(sf_client)
    ...
    print(f"\n🎉 Silicon Flow ERNIE-4.5语言学者分析完成!")
    print(f"📄 结果文件: {output_file}")
    print(f"💡 注意: 为了消除顺序偏差，每个需求的类别顺序都进行了随机打乱")
    print(f"📱 使用的平台: Silicon Flow")
    print(f"🤖 使用的模型: baidu/ERNIE-4.5-300B-A47B")
