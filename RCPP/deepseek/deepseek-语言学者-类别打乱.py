...
class DeepSeekProcessor:
    """DeepSeek模型处理器 - 语言学者角色"""

    def __init__(self, deepseek_client: DeepSeekClient):
        """初始化DeepSeek模型处理器"""
        self.deepseek_client = deepseek_client
        print(f"DeepSeek语言学者处理器初始化完成，使用模型: {deepseek_client.name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用DeepSeek模型进行预测（单次判断）
            prediction = self.deepseek_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"使用模型: {self.deepseek_client.name}")
...
def main():
    ...
    output_file = "deepseek语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("DeepSeek语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建DeepSeek模型客户端: {deepseek_client.name}")
        print(f"   API地址: https://api.deepseek.com/v1")
    except Exception as e:
        print(f"❌ 创建DeepSeek模型客户端失败: {e}")
        return
    ...
    processor = DeepSeekProcessor(deepseek_client)
    ...
    print(f"📱 使用的平台: DeepSeek")
    print(f"🤖 使用的模型: {deepseek_client.name}")
