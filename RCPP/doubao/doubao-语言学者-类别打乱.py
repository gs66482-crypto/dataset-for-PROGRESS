...
import re
import random
from volcenginesdkarkruntime import Ark  # 火山引擎方舟SDK
...
class DoubaoClient:
    ...
    def predict(self, text: str, labels: List[str],
        ...
        for attempt in range(3):  # 重试机制
            try:
                ...
                # 构建系统提示词和用户提示词
                system_prompt = self._build_system_prompt(shuffled_labels, shuffled_explanations, shuffled_examples)
    ...
    def _build_system_prompt(self, labels: List[str],
                             category_explanations: Dict[str, str] = None,
                             requirements_examples: Dict[str, List[str]] = None) -> str:
        """构建系统提示词 - 语言学者角色"""

        prompt = "你是一位资深的语言学者，专门研究软件需求的语言结构和表达方式。请从语言学角度分析软件需求语句的结构、用词、语法和语义特征，并给出最准确的分类和你的置信度评分。\n\n"

        # 构建清晰的分类标签部分（使用打乱的顺序）
        prompt += f"🏷️ 可选分类标签（请从以下标签中选择一个）:\n{', '.join(labels)}\n"
...
class DoubaoProcessor:
    ...
    def __init__(self, doubao_client: DoubaoClient):
        ...
        print(f"豆包语言学者处理器初始化完成，使用模型: {doubao_client.name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用豆包模型进行预测（单次判断）
            prediction = self.doubao_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"使用模型: {self.doubao_client.name}")
...
def main():
    ...
    output_file = "doubao语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("豆包语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建豆包模型客户端: {doubao_client.name}")
    except Exception as e:
        print(f"❌ 创建豆包模型客户端失败: {e}")
        return
    ...
    processor = DoubaoProcessor(doubao_client)
    ...
    print(f"📱 使用的平台: 字节跳动火山引擎方舟")
    print(f"🤖 使用的模型: {doubao_client.name}")
