...
class ResearchPaperConfidenceCalculator:
    """Calculate confidence exactly according to research paper method"""

    def __init__(self):
        """
        Strictly following paper formulas:
        C_base = V_top / N
    ...
    def calculate_confidence(self, vote_counts: Dict[str, int]) -> Dict[str, Any]:
        """
        Calculate confidence according to research paper method
        Returns dictionary containing calculation process
        """
        if not vote_counts:
            return {
...
class MultiRoleGLMClient:
    """Multi-role GLM model client class - Six professional roles based on research paper"""
    ...
    def __init__(self, name: str = "glm-4.5", model_name: str = "glm-4.5"):
        ...
        # Define six professional roles and their system prompts
        self.roles = {
            "product_owner": {
                "name": "Product Owner",
                "system_prompt": """You are a Product Owner focused on maximizing product value. You analyze requirements from the perspectives of business goals, market demand, return on investment (ROI), and product strategy. You focus on how requirements support business objectives, meet user needs, create business value, and determine requirement priorities. Your analysis focuses on the "why" behind developing a feature and its contribution to the product vision.""",
                "focus_areas": ["Business Value", "Market Demand", "ROI", "Product Strategy", "Priority"]
            },
            "business_analyst": {
                "name": "Business Analyst",
                "system_prompt": """You are a Business Analyst focused on requirements analysis, process optimization, and solution design. You analyze requirements from the perspectives of business processes, functional specifications, requirements completeness, and consistency. You focus on requirement clarity, testability, consistency with existing systems, and how to best meet business objectives. Your analysis focuses on "what" needs to be built and how to meet business requirements.""",
                "focus_areas": ["Process Analysis", "Functional Specifications", "Requirements Consistency", "Solution Design"]
            },
            "system_architect": {
                "name": "System Architect",
                "system_prompt": """You are a System Architect focused on technical feasibility, system design, and architectural constraints. You analyze requirements from the perspectives of technical implementation, scalability, performance, security, integration complexity, and maintenance cost. You focus on the technical impact of requirements, architectural decisions, technical debt, and long-term sustainability. Your analysis focuses on "how" to technically implement this requirement.""",
            ...
            "software_tester": {
                ...
                "system_prompt": """You are a Software Tester focused on quality assurance, test design, and defect prevention. You analyze requirements from the perspectives of testability, test case design, boundary conditions, error handling, and quality standards. You focus on how requirements can be verified, test coverage, potential risk points, and quality assurance measures. Your analysis focuses on how to "verify" that requirements are correctly implemented.""",
                "focus_areas": ["Testability", "Test Design", "Boundary Conditions", "Quality Assurance", "Verification Standards"]
            },
            "linguist": {
                "name": "Linguist",
                "system_prompt": """You are a Linguist specializing in language analysis, semantic interpretation, and communication clarity. You analyze requirements from the perspectives of linguistic precision, semantic ambiguity, terminology consistency, readability, and cross-cultural communication. You focus on how requirements are expressed, potential misinterpretations, language patterns, and whether the wording accurately conveys the intended meaning. Your analysis focuses on "how" requirements are articulated and understood through language.""",
                "focus_areas": ["Semantic Precision", "Terminology Consistency", "Readability", "Ambiguity Detection", "Cross-cultural Communication"]
            }
        }

        print(f"✅ Successfully created multi-role GLM model client: {self.name}")
        ...
        print(f"  API Configuration: Alibaba Cloud DashScope (https://dashscope.aliyuncs.com/compatible-mode/v1)")
        print(f"  Note: GLM-4.5 model only supports streaming mode (stream=True)")
    ...
    def predict_with_multi_roles(self, text: str, labels: List[str],
        ...
        """Use six professional roles for voting prediction - strictly following research paper method"""
        ...
        # Have each role vote in sequence
        for role_key, role_info in self.roles.items():
            ...
            for attempt in range(3):  # Retry mechanism
                try:
                    ...
                    # Call GLM API - must enable streaming mode
        ...
        # Calculate consensus confidence strictly according to research paper method
        ...
        print(f"  V_top = {confidence_details['v_top']}, V_second = {confidence_details['v_second']}, N = {confidence_details['total_votes']}")
        print(f"  C_base = V_top / N = {confidence_details['v_top']} / {confidence_details['total_votes']} = {confidence_details['c_base']:.2f}")
        print(f"  B_gap = min(0.2, (V_top - V_second) × 0.1) = min(0.2, ({confidence_details['v_top']} - {confidence_details['v_second']}) × 0.1) = {confidence_details['b_gap']:.2f}")
        print(f"  C_final = min(1.0, C_base + B_gap) = min(1.0, {confidence_details['c_base']:.2f} + {confidence_details['b_gap']:.2f}) = {final_confidence:.3f}")
...
Important: Ensure you return exactly the label name as shown, without modification, abbreviation, or any additional content."""
        ...
        if category_explanations:
            explanations_section = "\n\nCategory detailed explanations:\n"
            for label in labels:
...
class MultiRoleProcessor:
    ...
    def __init__(self, multi_role_client: MultiRoleGLMClient):
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("GLM-4.5 Model - Six Professional Role Classification System Analysis Report\n")
            f.write("=" * 80 + "\n\n")
            ...
            if confidences:
                ...
                for bin_range, count in confidence_bins.items():
                    if count > 0:
                        ...
                        f.write(f"  {bin_range}: {count} ({percentage:.1f}%)\n")
                ...
                f.write(f"\nVote Consistency Statistics:\n")
                f.write(f"  Unanimous (6:0): {unanimous} ({(unanimous / len(results) * 100):.1f}%)\n")
                f.write(f"  Absolute Majority (5:1): {majority_5} ({(majority_5 / len(results) * 100):.1f}%)\n")
                f.write(f"  Strong Majority (4:2): {majority_4} ({(majority_4 / len(results) * 100):.1f}%)\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"\n{'=' * 60}")
        print("📊 GLM-4.5 Model Six Professional Role Classification Statistics")
        print(f"{'=' * 60}")
        ...
        if confidences:
            ...
            print(f"  Medium Confidence (0.6-0.8): {medium_confidence_count} ({(medium_confidence_count / len(results) * 100):.1f}%)")
            print(f"  Low Confidence (<0.6): {low_confidence_count} ({(low_confidence_count / len(results) * 100):.1f}%)")
            print(f"  Unanimous (6:0): {unanimous_count} ({(unanimous_count / len(results) * 100):.1f}%)")
...
def main():
    ...
    output_file = "glm_6role_confidence.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("🎭 GLM-4.5 Model - Six Professional Role Software Requirement Classification System")
    print("Based on research paper method: Six professional role voting + Dual confidence calculation")
    ...
    # Create multi-role GLM model client
    try:
        multi_role_client = MultiRoleGLMClient(
            name="GLM-4.5-6Role",
            model_name="glm-4.5"
        )
    except Exception as e:
    ...
    print(f"\n🚀 Starting GLM-4.5 model six-role classification processing...")
    ...
    for i, data_point in enumerate(results[:3]):
        ...
        if data_point.prediction:
            ...
            if data_point.prediction.calculation_details:
                ...
                print(f"     C_final = min(1.0, {details['c_base']:.2f} + {details['b_gap']:.2f}) = {details['final_confidence']:.3f}")

    print(f"\n{'=' * 60}")
    print("🎉 GLM-4.5 Model Six-Role Classification Processing Complete!")
