...
class GLMProcessor:
    ...
    def __init__(self, glm_client: GLMClient):
        ...
        print(f"GLM语言学者处理器初始化完成，使用模型: {glm_client.name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用GLM模型进行预测（单次判断）
            prediction = self.glm_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"使用模型: {self.glm_client.name}")
...
def main():
    ...
    output_file = "glm语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("GLM语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建GLM模型客户端: {glm_client.name}")
    except Exception as e:
    ...
    processor = GLMProcessor(glm_client)
