...
class ResearchPaperConfidenceCalculator:
    ...
    def calculate_confidence(self, vote_counts: Dict[str, int]) -> Dict[str, Any]:
        ...
        N = 6  # Total number of votes fixed at 6
...
class MultiRoleGPTClient:
    """Multi-role GPT model client class - Based on research paper's six professional roles"""
    ...
    def __init__(self, name: str = "gpt-4.1-nano", model_name: str = "gpt-4.1-nano"):
        ...
        self.roles = {
            "product_owner": {
                "name": "Product Owner",
                "system_prompt": """You are a Product Owner, focused on maximizing product value. You analyze requirements from the perspectives of business goals, market demands, return on investment (ROI), and product strategy. You focus on how requirements support business objectives, meet user needs, create business value, and determine requirement priorities. Your analysis emphasizes "why" this feature should be developed and its contribution to the product vision.""",
            ...
            "system_architect": {
                ...
                "system_prompt": """You are a System Architect, focused on technical feasibility, system design, and architectural constraints. You analyze requirements from the perspectives of technical implementation, scalability, performance, security, integration complexity, and maintenance costs. You focus on the technical impact of requirements, architectural decisions, technical debt, and long-term sustainability. Your analysis emphasizes "how" to technically implement this requirement.""",
                "focus_areas": ["Technical Architecture", "Scalability", "Performance", "Security", "Integration"]
            },
            "user_experience_designer": {
                "name": "User Experience Designer",
                "system_prompt": """You are a User Experience Designer, focused on user interaction, interface design, and user experience optimization. You analyze requirements from the perspectives of user journey, interaction design, usability, accessibility, and emotional design. You focus on how requirements affect user satisfaction, ease of use, learning curve, and overall user experience. Your analysis emphasizes how requirements are "perceived and used by users.""",
                "focus_areas": ["User Experience", "Interaction Design", "Usability", "Accessibility", "User Satisfaction"]
            },
            "software_tester": {
                "name": "Software Tester",
                "system_prompt": """You are a Software Tester, focused on quality assurance, test design, and defect prevention. You analyze requirements from the perspectives of testability, test case design, boundary conditions, error handling, and quality standards. You focus on how requirements can be verified, test coverage, potential risk points, and quality assurance measures. Your analysis emphasizes how to "verify" whether requirements are correctly implemented.""",
                "focus_areas": ["Testability", "Test Design", "Boundary Conditions", "Quality Assurance", "Verification Standards"]
            },
            "linguist": {
                "name": "Linguist",
                "system_prompt": """You are a Linguist, focused on language clarity, ambiguity detection, terminology consistency, and communication effectiveness. You analyze requirements from the perspectives of linguistic precision, semantic clarity, pragmatic interpretation, cultural appropriateness, and technical writing quality. You focus on how requirements are expressed, potential misinterpretations, ambiguous phrasing, inconsistent terminology, and overall readability. Your analysis emphasizes the "linguistic quality and clarity" of requirements and how language affects understanding and implementation.""",
                "focus_areas": ["Language Clarity", "Ambiguity Detection", "Terminology Consistency", "Semantic Precision", "Technical Writing"]
            }
        }

        print(f"✅ Successfully created multi-role GPT model client: {self.name}")
        ...
        print(f"  Note: GPT-4.1-nano model only supports streaming mode (stream=True)")
    ...
    def predict_with_multi_roles(self, text: str, labels: List[str],
        ...
        # Let each role vote in sequence
        for role_key, role_info in self.roles.items():
            ...
            for attempt in range(3):  # Retry mechanism
                try:
                    ...
                    # Call GPT API - enable streaming mode
                    completion = self.client.chat.completions.create(
                        ...
                        temperature=0.3,
                        max_tokens=200,
                        stream=True  # GPT-4.1-nano only supports streaming mode
                    )

                    # Safely collect streaming response content
                    response_content = ""
                    for chunk in completion:
                        # Safely check chunk structure
...
class MultiRoleProcessor:
    ...
    def __init__(self, multi_role_client: MultiRoleGPTClient):
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("GPT-4.1-nano Model - Six Professional Role Classification System Analysis Report\n")
            f.write("=" * 80 + "\n\n")
            ...
            if confidences:
                ...
                f.write(f"  Strong Majority (5:1): {majority_5} items ({(majority_5 / len(results) * 100):.1f}%)\n")
                f.write(f"  Majority (4:2): {majority_4} items ({(majority_4 / len(results) * 100):.1f}%)\n")
                f.write(f"  Simple Majority (3:3): {majority_3} items ({(majority_3 / len(results) * 100):.1f}%)\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"\n{'=' * 60}")
        print("📊 GPT-4.1-nano Model - Six Professional Role Classification Statistics")
        print(f"{'=' * 60}")
...
def main():
    ...
    output_file = "gpt_6_roles_confidence.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("🎭 GPT-4.1-nano Model - Six Professional Role Software Requirement Classification System")
    ...
    # Create multi-role GPT model client
    try:
        multi_role_client = MultiRoleGPTClient(
            name="GPT-4.1-nano-6角色",
    ...
    print(f"\n🚀 Starting GPT-4.1-nano model six-role classification processing...")
    ...
    print(f"\n{'=' * 60}")
    print("🎉 GPT-4.1-nano model six-role classification processing complete!")
