...
class HunyuanClient:
    """腾讯混元模型客户端类 - 使用腾讯混元API"""

    def __init__(self, name: str = "hunyuan-turbos-latest", model_name: str = "hunyuan-turbos-latest"):
        self.name = name
...
class HunyuanProcessor:
    ...
    def __init__(self, hunyuan_client: HunyuanClient):
        """初始化腾讯混元模型处理器"""
        self.hunyuan_client = hunyuan_client
        print(f"腾讯混元语言学者处理器初始化完成，使用模型: {hunyuan_client.name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用腾讯混元模型进行预测（单次判断）
            prediction = self.hunyuan_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"使用模型: {self.hunyuan_client.name}")
...
def main():
    ...
    output_file = "hunyuan语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("腾讯混元语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建腾讯混元模型客户端: {hunyuan_client.name}")
        print(f"   API地址: {hunyuan_client.base_url}")
    except Exception as e:
        print(f"❌ 创建腾讯混元模型客户端失败: {e}")
        return
    ...
    processor = HunyuanProcessor(hunyuan_client)
    ...
    print(f"📱 使用的平台: 腾讯混元")
    print(f"🤖 使用的模型: {hunyuan_client.name}")
