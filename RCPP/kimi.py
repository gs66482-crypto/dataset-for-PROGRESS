...
class MultiRoleKimiClient:
    """多角色Kimi模型客户端类 - 基于研究论文的六种专业角色"""
    ...
    def __init__(self, name: str = "Moonshot-Kimi-K2-Instruct", model_name: str = "kimi-k2-turbo-preview"):
        ...
        self.roles = {
            ...
            "linguist": {
                "name": "Linguist",
                "system_prompt": """你是一位语言学家(Linguist)，专注于需求文本的语言表达、语义准确性和沟通清晰度。你从自然语言处理、语义分析、歧义性检测、术语一致性和表达规范性角度分析需求。你关注需求描述中的语言模糊性、潜在的理解歧义、术语使用的一致性以及整体表达的可读性。你的分析侧重于需求文本的"语言质量"如何影响需求的理解和沟通。""",
                "focus_areas": ["语义分析", "歧义检测", "术语一致性", "表达规范性", "可读性"]
            }
        }

        print(f"✅ 成功创建多角色Kimi模型客户端: {self.name}")
        ...
        print(f"  API配置: Moonshot (https://api.moonshot.cn/v1)")
...
class MultiRoleProcessor:
    ...
    def __init__(self, multi_role_client: MultiRoleKimiClient):
    ...
    def process_dataset(self, dataset: List[str], labels: List[str],
        ...
        print(f"开始处理 {len(dataset)} 条软件需求，使用6种专业角色...")
        ...
        for i, data_point in enumerate(results):
            ...
            row_data = {
                ...
                'Software Tester投票': data_point.prediction.role_votes.get('Software Tester',
                                                                            'N/A') if data_point.prediction and data_point.prediction.role_votes else 'N/A',
                'Linguist投票': data_point.prediction.role_votes.get('Linguist',
                                                                     'N/A') if data_point.prediction and data_point.prediction.role_votes else 'N/A',
    ...
    def _save_detailed_report(self, results: List[DataPoint], output_file: str):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("Kimi-K2模型-六人专业角色分类系统分析报告\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("6. Linguist (语言学家)\n\n")

            f.write("置信度计算公式（基于研究论文）:\n")
            f.write("C_base = V_top / N  (其中N=6)\n")
            ...
            if confidences:
                ...
                f.write(f"\n投票一致性统计:\n")
                f.write(f"  全票一致(6:0): {unanimous}条 ({(unanimous / len(results) * 100):.1f}%)\n")
                f.write(f"  绝对多数(5:1): {majority_5}条 ({(majority_5 / len(results) * 100):.1f}%)\n")
                f.write(f"  显著多数(4:2): {majority_4}条 ({(majority_4 / len(results) * 100):.1f}%)\n")
                f.write(f"  相对多数(3:3): {majority_3}条 ({(majority_3 / len(results) * 100):.1f}%)\n")
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        if confidences:
            ...
            print(f"  全票一致(6:0): {unanimous_count}条 ({(unanimous_count / len(results) * 100):.1f}%)")
...
def main():
    ...
    output_file = "kimi_6人2置信度.xlsx"  # 修改输出文件名

    data_loader = DataLoader()

    print("=" * 60)
    print("🎭 Kimi-K2模型-六人专业角色软件需求分类系统")
    print("基于研究论文方法: 六种专业角色投票 + 双置信度计算")
    ...
    # 创建多角色Kimi模型客户端
    try:
        multi_role_client = MultiRoleKimiClient(
