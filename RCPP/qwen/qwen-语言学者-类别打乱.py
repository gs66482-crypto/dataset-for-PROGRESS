...
class QwenProcessor:
    ...
    def __init__(self, qwen_client: QwenClient):
        ...
        print(f"Qwen语言学者处理器初始化完成，使用模型: {qwen_client.name}")
    ...
    def process_dataset(self, dataset: List[str], true_labels: List[str], labels: List[str],
        ...
        for i, (text, true_label) in enumerate(zip(dataset, true_labels)):
            ...
            # 使用Qwen模型进行预测（单次判断）
            prediction = self.qwen_client.predict(text, labels, category_explanations, requirements_examples)
    ...
    def print_statistics(self, results: List[DataPoint]):
        ...
        print(f"使用模型: {self.qwen_client.name}")
...
def main():
    ...
    output_file = "qwen语言学者1次分类结果_随机顺序.xlsx"

    data_loader = DataLoader()

    print("=" * 60)
    print("Qwen语言学者软件需求分类系统（随机顺序版）")
    print("=" * 60)
    print("模式: 语言学者角色，单次判断，使用大模型自评置信度")
    ...
    try:
        ...
        print(f"✅ 成功创建Qwen模型客户端: {qwen_client.name}")
    except Exception as e:
        print(f"❌ 创建Qwen模型客户端失败: {e}")
        return
    ...
    processor = QwenProcessor(qwen_client)
